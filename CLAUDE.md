...
| `mcp_servers/memory_server.py` | MCP server exposing vault read/write as tools |
...
- **Orchestrator routing**: Simple keyword matching (`build`/`scan` → build pipeline, `stats` → stats, else → query). Intent logic lives in `orchestrator.py:route()`.
...
| `QueryAgent` | Any other message | Calls `search_vault` + `read_memory` MCP tools, synthesizes answer |
